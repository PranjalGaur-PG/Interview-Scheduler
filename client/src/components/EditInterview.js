...
function EditInterview() {
  ...
  const clickHandler = async (e) => {
    ...
    const epochSDate = sDate.getTime();
    ...
    if (
      ...
      alert("Enter the information carefully !");
      return;
    }
  ...
  useEffect(() => {
    async function readData() {
      ...
      const interview = await axios.get(`/api/interviews/read/${id}`);
      // console.log(interview);

      const Participants = interview.data.participants;

      Promise.all(
        Participants.map(async (participant) => {
          const name = await axios.get(`/api/users/read/${participant}`);
          // console.log(name.data);
          setSelectedUsers((prev) => [...prev, name.data]);
        })
      );

      setEDate(new Date(parseInt(interview.data.eDate)));
      setSDate(new Date(parseInt(interview.data.sDate)));
      setTopic(interview.data.topic);
  ...
  return isLoading ? (
    <div className="flex flex-col items-center justify-center bg-slate-200">
      <h1 className="mx-16 mt-16 text-5xl">Edit Interview</h1>
      <Loader />
    </div>
  ) : (
    <div className="flex flex-col items-center justify-center bg-slate-200">
      <div className="p-5 ml-auto mr-10">* Required fields</div>
      <h1 className="mx-16 text-5xl">Edit Interview</h1>
